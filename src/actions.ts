...
export class Love implements IAction {
	...
	constructor(
		...
		if (character.species === SPECIES.PURPLUS) {
			this.offspring_sprite = new RandomSprite(
				partner.pic,
				partner.anims.offspring.soffset,
				purplus_special_animations.offspring.frames,
				purplus_special_animations.offspring.transitions,
				anim_delays.offspring,
			);
		} else {
			this.offspring_sprite = new Sprite(
				partner.pic,
				partner.anims.offspring.soffset,
				partner.anims.offspring.frames,
				anim_delays.offspring,
			);
		}
		...
		switch (dir) {
			case DIR.N:
				...
				this.cloud_sprite = new Sprite(
					cloud_pic,
					anims_clouds.love_vert.soffset,
					anims_clouds.love_vert.frames,
					0,
					anims_clouds.love_vert.size,
					true,
				);
			...
			case DIR.W:
				...
				this.cloud_sprite = new Sprite(
					cloud_pic,
					anims_clouds.love_hor.soffset,
					anims_clouds.love_hor.frames,
					0,
					anims_clouds.love_hor.size,
					true,
				);
		...
		if (this.frame === 2) {
			...
			if (this.character.species === SPECIES.PURPLUS) {
				this.sprites.push(
					new RandomSprite(
						this.partner.pic,
						this.partner.anims.offspring.soffset,
						purplus_special_animations.offspring.frames,
						purplus_special_animations.offspring.transitions,
					),
				);
			} else if (this.partner.anims.hasOwnProperty('pre_offspring')) {
				this.pre_offspring = new Sprite(
					this.partner.pic,
					this.partner.anims.pre_offspring.soffset,
					this.partner.anims.pre_offspring.frames,
					0,
					[64, 64],
					true,
				);
	...
	render(ctx: CanvasRenderingContext2D, dim: Dimension, cpos: Point) {
		for (let i = 0; i < this.sprites.length; i++) {
			this.sprites[i].render(ctx, [Math.round(this.tiles[i][0] * dim[0] - cpos[0]), Math.round(this.tiles[i][1] * dim[1] - cpos[1])]);
		}
...
export class Fight implements IAction {
	...
	constructor(
		...
		switch (dir) {
			case DIR.N:
				...
				this.cloud_sprite = new Sprite(
					cloud_pics,
					anims_clouds.fight_vert.soffset,
					anims_clouds.fight_vert.frames,
					0,
					anims_clouds.fight_vert.size,
				);
				this.cloud_offset = [0, 14];
				break;
			case DIR.S:
			...
			case DIR.W:
				...
				this.cloud_sprite = new Sprite(
					cloud_pics,
					anims_clouds.fight_hor.soffset,
					anims_clouds.fight_hor.frames,
					0,
					anims_clouds.fight_hor.size,
				);
				this.cloud_offset = [14, 0];
				break;
			default: // DIR.E
		...
		switch (this.frame) {
			...
			case 20:
				...
				if (this.player_wins) {
					...
					this.sprites = [null, new Sprite(this.opponent.pic, this.opponent.anims.winner.soffset, this.opponent.anims.winner.frames)];
				}
	...
	render(ctx: CanvasRenderingContext2D, dim: Dimension, cpos: Point) {
		for (let i = 0; i < this.sprites.length; i++) {
			...
			if (sprite !== null) {
				sprite.render(ctx, [Math.round(this.tiles[i][0] * dim[0] - cpos[0]), Math.round(this.tiles[i][1] * dim[1] - cpos[1])]);
			}
		}
...
export class Feeding implements IAction {
	...
	constructor(
		...
		this.sprite = new Sprite(
			character.pic,
			anims_players[character.species].feeding.soffset,
			anims_players[character.species].feeding.frames,
			0,
			[64, 64],
			true,
		);
	...
	update() {
		...
		// This is depended on the species, so no fixed frame
			...
			// Poison
				...
				this.sprite = new Sprite(
					this.character.pic,
					anims_players[this.character.species].poisoned.soffset,
					anims_players[this.character.species].poisoned.frames,
					0,
					[64, 64],
					true,
				);
			...
			// Power food
				...
				this.sprite = new Sprite(
					this.character.pic,
					anims_players[this.character.species].power_food.soffset,
					anims_players[this.character.species].power_food.frames,
					0,
					[64, 64],
					true,
				);
	...
	render(ctx: CanvasRenderingContext2D, dim: Dimension, cpos: Point) {
		this.sprite.render(ctx, [Math.round(this.tiles[0][0] * dim[0] - cpos[0]), Math.round(this.tiles[0][1] * dim[1] - cpos[1])]);
	}
}
...
export class Quicksand implements IAction {
	...
	constructor(character: Character, callback: () => void, resources: ResourceManager) {
		this.character = character;
		this.callback = callback;
		this.delay = this.character.species === SPECIES.PESCIODYPHUS ? anim_delays.feeding : (this.delay = anim_delays.quicksand);
...
export class Electro implements IAction {
	...
	update() {
		...
		if (this.frame === 15) {
			this.sprites[sprite_pos] = new Sprite(
				this.character.pic,
				this.character.anims.zapped.soffset,
				this.character.anims.zapped.frames,
				anim_delays.electro,
				[64, 64],
				true,
			);
