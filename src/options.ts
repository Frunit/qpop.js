...
export class Options implements Stage {
	...
	redraw() {
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_lang,
			[this.text_x, this.ys.lang + this.text_y_offset + 3],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_this_lang,
			[this.secondary_x_offset + this.lang_dim[0] / 2, this.ys.lang + this.text_y_offset + 3],
			'white',
			'black',
		);
		this.clickareas.push({
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_music,
			[this.text_x, this.ys.music + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_sound,
			[this.text_x, this.ys.sound + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_ai_speed,
			[this.text_x, this.ys.ai_speed + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		this.draw_bar(
			[this.secondary_x_offset, this.ys.ai_speed + this.line_height],
			Math.ceil((100 * this.glob.options.wm_ai_delay_idx) / (this.wm_ai_delays.length - 1)),
		);
		write_text(
			this.glob.ctx,
			this.glob.lang.options_ai_speeds[this.glob.options.wm_ai_delay_idx],
			[this.text_x, this.ys.ai_speed + this.text_y_offset + this.line_height],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_auto_continue,
			[this.text_x, this.ys.auto_continue + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_click_hold,
			[this.text_x, this.ys.click_hold + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_plants,
			[this.text_x, this.ys.plants + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_predators,
			[this.text_x, this.ys.predators + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_tutorial,
			[this.text_x, this.ys.tutorial + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_transition,
			[this.text_x, this.ys.transition + this.text_y_offset],
			'#000000',
			'#ffffff',
			'left',
		);
		write_text(
			this.glob.ctx,
			`${(this.glob.options.update_freq * this.glob.options.transition_delay).toFixed(2)} s`,
			[this.text_x, this.ys.transition + this.text_y_offset + this.line_height],
			'#000000',
			'#ffffff',
			'left',
		);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.options_restart,
			[this.text_x + this.lang_dim[0] / 2, this.ys.restart + this.text_y_offset + 3],
			'#ffffff',
			'#000000',
			'center',
		);
		...
		this.clickareas.push({
			...
			up: () =>
				open_popup(
					this.game,
					'chuck_berry',
					this.glob.lang.really_restart,
					(x: number) => this.restart_game(x),
					this.glob.lang.no,
					this.glob.lang.yes,
				),
	...
	draw_bar(pos: Point, percent: number) {
		...
		if (length > 0) {
			...
			this.glob.ctx.drawImage(
				this.bar,
				this.bar_soffset[0] + this.bar_dim[0] - this.last_bit_width,
				this.bar_soffset[1],
				this.last_bit_width,
				this.bar_dim[1],
				pos[0] + this.plusminus_dim[0] + length - 1,
				pos[1],
				this.last_bit_width,
				this.bar_dim[1],
			);
	...
	draw_plus(pos: Point) {
		this.glob.ctx.drawImage(
			...
			pos[0] + this.plusminus_dim[0] + this.bar_dim[0] - 2,
			pos[1],
			this.plusminus_dim[0],
			this.plusminus_dim[1],
		);
	...
	change_transition_delay(value: number) {
		...
		this.glob.ctx.drawImage(
			this.bg,
			this.text_x - 1,
			this.ys.transition + this.line_height - 1,
			this.secondary_x_offset,
			this.text_y_offset + 2,
			this.text_x - 1,
			this.ys.transition + this.line_height - 1,
			this.secondary_x_offset,
			this.text_y_offset + 2,
	...
	change_ai_speed(value: number) {
		...
		this.glob.ctx.drawImage(
			this.bg,
			this.text_x - 1,
			this.ys.ai_speed + this.line_height - 1,
			this.secondary_x_offset,
			this.text_y_offset + 6,
			this.text_x - 1,
			this.ys.ai_speed + this.line_height - 1,
			this.secondary_x_offset,
			this.text_y_offset + 6,
