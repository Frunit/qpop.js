...
export class Load implements Stage {
	...
	init_upload(e: MouseEvent) {
		...
		if (
			pos_x >= this.offset[0] + this.upload_offset[0] &&
	...
	redraw() {
		...
		draw_rect(
			this.glob.ctx,
			[this.offset[0] + this.upload_area_offset[0], this.offset[1] + this.upload_area_offset[1]],
			this.upload_area_dim,
		);
		...
		for (let i = 0; i < lines.length; i++) {
			write_text(
				...
				lines[i],
				[
					this.offset[0] + this.upload_offset[0] + this.upload_dim[0] / 2,
					this.offset[1] + this.upload_text_offset[1] - line_correction + this.line_height * i,
				],
				'white',
				'black',
			);
		...
		for (let i = 0; i < lines.length; i++) {
			write_text(
				this.glob.ctx,
				lines[i],
				[this.offset[0] + this.upload_text_offset[0], this.offset[1] + this.upload_text_offset[1] - line_correction + this.line_height * i],
				'white',
				'black',
				'left',
			);
		...
		this.clickareas.push({
			...
			down: () =>
				draw_rect(
					this.glob.ctx,
					[this.offset[0] + this.upload_offset[0], this.offset[1] + this.upload_offset[1]],
					this.upload_dim,
					true,
					true,
				),
		...
		draw_rect(
			this.glob.ctx,
			[this.offset[0] + this.browser_area_offset[0], this.offset[1] + this.browser_area_offset[1]],
			this.browser_area_dim,
		);
		...
		if (!Array.isArray(save_array) || save_array[0] === null) {
			...
			for (let i = 0; i < lines.length; i++) {
				write_text(
					...
					lines[i],
					[
						this.offset[0] + this.dim[0] / 2,
						this.offset[1] + this.browser_area_offset[1] + this.browser_area_dim[1] / 2 - line_correction + this.button_dim[1] * i,
					],
					'white',
					'black',
				);
			...
			for (let i = 0; i < save_array.length; i++) {
				...
				draw_rect(
					this.glob.ctx,
					[this.offset[0] + this.saves_offset[0], this.offset[1] + this.saves_offset[1] + this.button_y_dist * i],
					this.button_dim,
				);
				write_text(
					this.glob.ctx,
					this.glob.lang.load,
					[
						this.offset[0] + this.saves_offset[0] + this.button_dim[0] / 2,
						this.offset[1] + this.saves_offset[1] + this.button_y_dist * i + 15,
					],
					'white',
					'black',
				);
				...
				write_text(
					this.glob.ctx,
					`${new Date(save_array[i].datetime).toLocaleString()} - ${this.glob.lang.turn} ${save_array[i].turn}`,
					[this.offset[0] + this.browser_text_offset[0], this.offset[1] + this.browser_text_offset[1] + this.button_y_dist * i + 15],
					'white',
					'black',
					'left',
				);
		...
		write_text(
			this.glob.ctx,
			this.glob.lang.close,
			[this.offset[0] + this.abort_offset[0] + this.abort_dim[0] / 2, this.offset[1] + this.abort_offset[1] + 15],
			'white',
			'black',
		);
		...
		this.clickareas.push({
			...
			down: () =>
				draw_rect(
					this.glob.ctx,
					[this.offset[0] + this.abort_offset[0], this.offset[1] + this.abort_offset[1]],
					this.abort_dim,
					true,
					true,
				),
