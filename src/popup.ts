	...
	redraw() {
		...
		for (let i = 0; i < this.text.length; i++) {
			write_text(
				this.glob.ctx,
				this.text[i],
				[this.offset[0] + this.text_offset[0], this.offset[1] + this.text_offset[1] - line_correction + this.line_height * i],
				'white',
				'black',
			);
		}
		...
		if (this.left_answer) {
			draw_rect(
				this.glob.ctx,
				[this.offset[0] + this.left_answer_offset[0], this.offset[1] + this.left_answer_offset[1]],
				this.left_answer_dim,
			);
			write_text(
				this.glob.ctx,
				this.left_answer,
				[this.offset[0] + this.left_answer_offset[0] + this.left_answer_dim[0] / 2, this.offset[1] + this.left_answer_offset[1] + 15],
				'white',
				'black',
			);
		...
		draw_rect(
			this.glob.ctx,
			[this.offset[0] + this.right_answer_offset[0], this.offset[1] + this.right_answer_offset[1]],
			this.right_answer_dim,
		);
		write_text(
			this.glob.ctx,
			this.right_answer,
			[this.offset[0] + this.right_answer_offset[0] + this.right_answer_dim[0] / 2, this.offset[1] + this.right_answer_offset[1] + 15],
			'white',
			'black',
		);
		...
		if (!this.left_answer) {
			...
			this.glob.ctx.lineTo(
				this.offset[0] + this.right_answer_offset[0] + this.right_answer_dim[0] - 1,
				this.offset[1] + this.right_answer_offset[1] - 1,
			);
